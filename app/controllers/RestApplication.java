  ...
  public static Result updateUser() {
    ...
    if (!authenticatedUser.isDefined()
        || !User.class.isAssignableFrom(authenticatedUser.get()
            .getClass())) {
      return badRequest(play.libs.Json.toJson(new Notification.Error(
          "A user needs to be authenticated in order to perform an update")));
    }
    JsonNode json = request().body().asJson();
    if (json.findPath("email") == null) {
      return badRequest(play.libs.Json.toJson(new Notification.Error(
          "An email needs to be present in order to perform the update")));
    }
    String email = json.findPath("email").asText();
    User toBeUpdated = User.findByEmail(email);
    if (toBeUpdated == null) {
      return badRequest(play.libs.Json.toJson(new Notification.Error("No user with email '" + email + "' found.")));
    }
    toBeUpdated.setFirstName(json.findPath("firstName").asText());
    toBeUpdated.setLastName(json.findPath("lastName").asText());
    toBeUpdated.setShoesMeasure(new java.math.BigDecimal(json.findPath("shoesMeasure").asDouble()));
    Ebean.save(toBeUpdated);
    return ok(play.libs.Json.toJson(new Notification.Success(
          "Update performed successfully", play.libs.Json.toJson(toBeUpdated))));
  }
