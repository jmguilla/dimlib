...
public class MyUserService extends BaseUserService {
  ...
  public Identity doSave(Identity user) {
    ...
    toSave.setEmail(user.email().get());
  ...
  private Identity updateUser(Identity user, User toUpdate) {
    Logger.debug("Updating user: " + toUpdate.getEmail());
  ...
  private User recopyUser(Identity user, User toUpdate) {
    ...
    if (User.AUTH_USERPASS.equalsIgnoreCase(user.id().providerId())) {
      toUpdate.setPassword(user.passwordInfo().get().password());
      toUpdate.setHasher(user.passwordInfo().get().hasher());
      toUpdate.setSalt((user.passwordInfo().get().salt().isDefined() ? user
          .passwordInfo().get().salt().get()
          : null));
    }
    toUpdate.setFirstName(user.firstName());
    toUpdate.setFullName(user.fullName());
    toUpdate.setLastName(user.lastName());
    toUpdate.setProviderId(user.id().providerId());
    toUpdate.setUserId(user.id().id());
    toUpdate.setAvatarUrl((user.avatarUrl().isDefined() ? user.avatarUrl()
        .get() : null));
    return toUpdate;
  }
  ...
  private User cacheUser(User user) {
    return user;
    // TODO enable caching... needs to interopearte with normal users update mecanism
    // Logger.debug("Caching user: " + user.getEmail());
    // idToUserToUser.put(user.getUserId(), user);
    // emailAndPidToUserToUser.put(
    // User.buildEmailColonProviderId(user.getEmail(), user.getProviderId()),
    // user);
