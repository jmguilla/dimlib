...
public class MyUserService extends BaseUserService {
  ...
  public Identity doSave(Identity user) {
    ...
    alreadyRegistered = User.findByEmailAndProvider(user.email().get(), user.id().providerId());
    if (alreadyRegistered != null) {
      return updateUser(user, alreadyRegistered);
    }
    alreadyRegistered = User.findByUserIdAndProviderId(user.id().id(), user.id().providerId());
    if (alreadyRegistered != null) {
      return updateUser(user, alreadyRegistered);
    }
  ...
  public Identity doFind(UserId userId) {
    ...
    result = User.findByUserIdAndProviderId(userId.id(), userId.providerId());
  ...
  public Identity doFindByEmailAndProvider(String email, String providerId) {
    User result = null;
    if ((result = emailAndPidToUserToUser.get(User.buildEmailColonProviderId(email, providerId))) != null) {
      return result;
    }
  ...
  private User cacheUser(User user) {
    ...
    emailAndPidToUserToUser.put(User.buildEmailColonProviderId(user.email, user.providerId), user);
